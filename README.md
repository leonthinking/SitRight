...
热力图的绿色深浅表示当天相对活动目标的完成度，而不是固定活动次数；完整绿色表示已经达标。今天使用强调描边，已记录为暂停日或非工作日的日期以中性描边显示，首尾用于对齐星期的范围外格子不会绘制。没有历史目标快照的旧活动只使用最浅绿色，避免把未知完成度误报为达标。当前没有保存首次追踪日期，因此安装前无数据和安装后未活动的日期暂时仍使用相同的基础灰色。
