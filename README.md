...
- 可在「设置 → 通用 → 应用 → 语言」中切换简体中文或 English；菜单栏面板、设置、提醒引导、系统通知和桌面小组件会同步更新
...
应用内语言偏好默认保持简体中文，并保存在现有设置文件中。切换语言只改变展示文案，不会重新计时、改变活动统计、改变系统的 12/24 小时制或迁移活动历史；Widget 会通过 App Group 快照使用同一语言。Dock 与应用菜单中的应用名称固定为「SitRight 坐正」，不会随应用内语言切换；macOS 自带菜单命令和 Sparkle 标准更新窗口中的系统级文案仍可能遵循系统语言，也无法随应用内偏好即时变化。
...
组件正文会跟随 SitRight 的应用内语言偏好显示简体中文或英文；在应用中切换后会刷新共享快照和组件时间线。组件库自身的名称和系统按钮仍由 macOS 的系统语言决定。
